...
## Recipe 2 — Semantic fence, all auditors, full reports

A real fence assessment: every auditor, all four report formats, a global attack cap
with a higher per-auditor cap for garak. The judge model is set because some auditors
grade with a model even against a fence.
...
# Run all four auditors (omit AUDITORS entirely for the same effect)
PENTESTER_AUDITORS=["garak","pyrit","inspect_ai","promptfoo"]

# Cost control: 50 attacks each, but let garak go deeper
PENTESTER_MAX_ATTACKS=50
PENTESTER_GARAK__MAX_ATTACKS=200

# Judge model (used by model-graded auditors)
...
> `promptfoo` shells out to an external CLI and reaches a remote generation service —
> it needs **network access**. See [Troubleshooting → promptfoo](./troubleshooting.md#promptfoo-fails-or-hangs).

---

## Recipe 3 — LLM target graded by a judge
...
> [Core Concepts → The judge is not the target](./concepts.md#the-judge-is-not-the-target).
...
## Recipe 4 — Promptfoo, the full set of knobs

Promptfoo has the most settings of any auditor because it runs a full
**generate → evaluate** pipeline: it writes per-plugin test files, expands each into
adversarial prompts, then evaluates them. This recipe runs **only** promptfoo and uses
settings straight from the [Promptfoo auditor reference](../promptfoo.md#configuration).

### What model does promptfoo use?

Promptfoo takes no per-model env var here — it needs an **LLM API key**, and the model
that authors the attacks and grades responses comes from its own config template
(`redteam.provider`, `openai:gpt-4o-mini` by default). On your side you just supply the
key:

- **`SEMANTIC_FENCE` target:** the single-turn pass generates attacks remotely and
  grades with JavaScript assertions, so a key isn't strictly required — but
  **multi-turn always needs one** (an LLM drives each follow-up turn).
- **`LLM` target:** a key is always required (responses are graded by `llm-rubric`).

### Complete `.env.local`

```bash
# .env.local  — promptfoo against a semantic fence, multi-turn enabled

PENTESTER_TARGET_TYPE=SEMANTIC_FENCE
PENTESTER_SCANNER__CURL_COMMAND=curl -X POST 'https://api.example.com/chat' -H 'Content-Type: application/json' --data-raw '{"text": $PROMPT}'
PENTESTER_SCANNER__JSON_DOT_TARGET=body.valid

PENTESTER_AUDITORS=["promptfoo"]

# --- Test volume: generation stage --------------------------------------
PENTESTER_PROMPTFOO__MAX_TEST_FILES=20       # cap generated test YAMLs (unset = all)
PENTESTER_PROMPTFOO__PLUGINS_PER_FILE=1      # plugins bundled per file (1–5)
PENTESTER_PROMPTFOO__PLUGIN_NUM_TESTS=5      # override numTests per plugin

# --- Test volume: evaluation stage --------------------------------------
PENTESTER_PROMPTFOO__MAX_TESTS=2000          # cap attacks executed per file (-n)
PENTESTER_PROMPTFOO__MAX_ATTACKS=100         # overall attack limit
PENTESTER_PROMPTFOO__FILES_PARALLEL=5        # test files evaluated in parallel

# --- Multi-turn jailbreaks (opt-in; always needs an LLM key) ------------
PENTESTER_PROMPTFOO__ENABLE_MULTITURN=true
PENTESTER_PROMPTFOO__MULTITURN_STRATEGIES=["crescendo","goat","mischievous-user"]
PENTESTER_PROMPTFOO__MULTITURN_MAX_TURNS=5          # 1–20
PENTESTER_PROMPTFOO__MULTITURN_MAX_BACKTRACKS=5     # 1–20 (crescendo only)
PENTESTER_PROMPTFOO__MULTITURN_STATEFUL=false       # false = stateless; full history resent each turn
PENTESTER_PROMPTFOO__MULTITURN_CONTINUE_AFTER_SUCCESS=false

# --- LLM API key (required for multi-turn; and for LLM targets) ---------
OPENAI_API_KEY=sk-...
...
### Every promptfoo setting

The full set, straight from the [Promptfoo reference](../promptfoo.md#promptfoo-settings).
The generation settings (`MAX_TEST_FILES`, `PLUGINS_PER_FILE`, `PLUGIN_NUM_TESTS`) shape
the files written to disk; the evaluation setting (`MAX_TESTS`) caps how many attacks are
actually executed.

| Variable | Default | Description |
|---|---|---|
| `PENTESTER_PROMPTFOO__CONFIG_PATH` | `./pentester/config/auditor_files/promptfoo` | Path to the promptfoo config directory. |
...
| `PENTESTER_PROMPTFOO__PLUGIN_NUM_TESTS` | `None` | Override `numTests` per plugin; `None` uses the base config value. |
...
| `PENTESTER_PROMPTFOO__ASSERTION_WRAPPER_PATH` | `None` | Path to a custom assertion Python file. When set, a Python assertion is used; when `None`, JavaScript assertions are used. |
| `PENTESTER_PROMPTFOO__ENABLE_MULTITURN` | `false` | Enable the multiturn evaluation pass. |
...
| `PENTESTER_PROMPTFOO__MULTITURN_STRATEGIES` | all 3 | JSON list of strategies, e.g. `["crescendo","goat"]`. |

Plus the global `PENTESTER_TARGET_TYPE` (`SEMANTIC_FENCE` or `LLM`), which decides the
assertion style and API-key handling.

### Which multi-turn setting applies to which strategy

Each strategy honors a different subset of the multiturn knobs:

| Setting | `crescendo` | `goat` | `mischievous-user` |
|---|:---:|:---:|:---:|
| `MULTITURN_MAX_TURNS` | ✓ | ✓ | ✓ |
...
> `jailbreak:hydra` appears in some older docs but is **disabled** — it requires
> Promptfoo Cloud. The available strategies are `crescendo`, `goat`, and
> `mischievous-user`.

---

## Recipe 5 — PyRIT multi-turn (Crescendo + Tree of Attacks)

PyRIT's strength is multi-turn manipulation. This runs two multi-turn strategies with a
shared objective and a scorer LLM.
...
PENTESTER_SCANNER__CURL_COMMAND=curl -X POST 'https://api.example.com/chat' -H 'Content-Type: application/json' --data-raw '{"text": $PROMPT}'
PENTESTER_SCANNER__JSON_DOT_TARGET=body.valid
# Multi-turn needs the reply text to feed the conversation
PENTESTER_SCANNER__RESPONSE_TEXT_TARGET=body.message

PENTESTER_AUDITORS=["pyrit"]

# Which seed datasets to draw from (JSON array; empty = all)
PENTESTER_PYRIT__DATASET_NAMES=["xstest"]
...
PENTESTER_PYRIT__MAX_BACKTRACKS=10        # crescendo
PENTESTER_PYRIT__TREE_WIDTH=3             # tree_of_attacks
PENTESTER_PYRIT__TREE_DEPTH=5             # tree_of_attacks
PENTESTER_PYRIT__BRANCHING_FACTOR=2       # tree_of_attacks
PENTESTER_PYRIT__MAX_ATTACKS=20

# Scorer LLM (required for multi-turn, even against a fence)
PENTESTER_LLM__PROVIDER=openai
PENTESTER_LLM__MODEL=gpt-4o-mini
OPENAI_API_KEY=sk-...
...
Valid `ATTACK_STRATEGIES`: `multi_prompt_sending` (default, single-turn),
`red_teaming`, `crescendo`, `tree_of_attacks`.
...
## Recipe 6 — Custom handler (no curl)

When your target isn't reachable by plain `curl` — a Python SDK, gRPC, request
signing, a local model — point the scanner at a `CustomHandler` class instead. Bypass
detection lives inside the handler (you set `passed`), so there's no `json_dot_target`.
...
The handler (see [Core Concepts → Custom handlers](./concepts.md#custom-handlers) for
the contract):
...
class MyServiceHandler(CustomHandler):
    def request(self, text: str) -> HandlerResponse:
        reply = my_client.send(text)
        return HandlerResponse(
            response=reply.raw_text,
...
## Recipe 7 — Inspect AI benchmarks

Runs specific Inspect AI evaluation suites with a dedicated judge model.
...
PENTESTER_AUDITORS=["inspect_ai"]

# Which benchmarks to run (JSON array; empty = auto-select by target type)
PENTESTER_INSPECT__EVALS=["strong_reject","b3"]
PENTESTER_INSPECT__EPOCHS=1
PENTESTER_INSPECT__LIMIT=50          # cap samples per eval (unset = full dataset)
...
## Recipe 8 — Everything at once (all auditors, all settings)
...
- **Two advanced promptfoo paths are commented out on purpose.** `CONFIG_PATH` and
...
- [Full run walkthrough](./full-run.md) — the narrative version of these recipes.
- [Core Concepts](./concepts.md) — target types, the judge/target split, bypass detection.
- [Configuration reference](../configuration.md) — the authoritative per-variable table.
- [Troubleshooting](./troubleshooting.md) — when a scan won't start or a report is empty.
