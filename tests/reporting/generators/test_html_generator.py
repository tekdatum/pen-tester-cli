...
def test_generate_summary_report_returns_none() -> None:
    result = HtmlGenerator().generate_summary_report({"host": "10.0.0.1"}, "/tmp/out")
    assert result is None


def test_generate_details_report_returns_none() -> None:
    result = HtmlGenerator().generate_details_report([_probe()], "/tmp/out")
    assert result is None


def test_generate_details_report_accepts_empty_list() -> None:
    result = HtmlGenerator().generate_details_report([], "/tmp/out")
    assert result is None
